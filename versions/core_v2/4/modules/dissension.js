...
export function addhome(action, id) {
	try {
		var is = document.getElementById(id);
	} catch { }
	if (is)
		return; // only run if it doesnt exist
...
export function usertoken() {
	...
	if (!popup)
		return alert('Popup blocked! Please allow popups and try again.');
...
export function sendmsg(message, embed, channelId) {
	if (!channelId) {
